...
public class SampleODataClient {
  ...
  /*
  ClientEntity loadEntity(String path) throws ODataDeserializerException {
  ...
  public Future<ODataMediaEntityUpdateResponse<ClientEntity>> updateEntityMedia(String serviceUri, String entityName, Object keyValue, InputStream inputStream) {
    URI absoluteUri = client.newURIBuilder(serviceUri).appendEntitySetSegment(entityName)
            .appendKeySegment(keyValue).build();
    ODataMediaEntityUpdateRequest<ClientEntity> request =
            client.getCUDRequestFactory().getMediaEntityUpdateRequest(absoluteUri, inputStream);
    MediaEntityUpdateStreamManager<ClientEntity> manager = request.payloadManager();
    final var asyncResponse = manager.getAsyncResponse();
